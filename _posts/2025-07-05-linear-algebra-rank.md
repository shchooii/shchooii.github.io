...
행렬 공간의 개념을 확장하고, 부분 공간의 차원 계산, 랭크 1 행렬의 의미, 그리고 스몰 월드 그래프를 다룹니다.

---

### Matrix as a Vector Space 📐

행렬(matrix)도 벡터(vector)가 될 수 있다는 아이디어에서 시작합니다. 
...
이 두 가지 조건을 만족하므로, 모든 3x3 행렬의 집합 `M`은 그 자체로 하나의 거대한 벡터 공간이 됩니다. 이 공간의 차원(dimension)은 행렬의 원소 개수인 9입니다.
...
랭크 1 행렬은 모든 행렬을 구성하는 가장 기본적인 "벽돌"과 같습니다. 이는 하나의 열벡터와 하나의 행벡터의 곱으로 표현됩니다.
