...
이 세 가지를 조합하면 개발–테스트–운영 파이프라인을 빠르게 구축할 수 있습니다.
...
> Compose에서 `driver: bridge`를 지정하면 위에서 만든 `foodtrucks-net`과 같은 사용자 정의 브리지가 생성됩니다.
...
> Dockerfile로 이미지를 표준화하고, Docker Compose로 다중 서비스를 선언적으로 관리하면
